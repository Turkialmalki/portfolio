#!/usr/bin/env bash
# Command 05C — REAL AI synthetic fixture suite.
#
# Run this yourself with ADMIN_API_KEY exported in your terminal (never paste
# the key itself back to me — only the JSON this script prints is needed).
#
#   export ADMIN_API_KEY=<your value>
#   bash real-ai-fixture-suite.sh | tee real-ai-results.json
#
# Then paste the contents of real-ai-results.json back into the chat.
#
# All fixtures are the FICTIONAL resumes already checked into
# supabase/tests/analysis/fixtures.ts — no real CV content, nothing new.
# Each call uses mode:"fixture_test" against the deployed analyze-resume
# function, which now (Command 05C fix) resolves the REAL Anthropic
# provider since AI_PROVIDER_API_KEY is configured on the project.
...
post() {
  ...
  ms=$((end - start))
  echo "{\"fixture\":\"$name\",\"wall_clock_ms\":$ms,\"response\":$resp}"
}
